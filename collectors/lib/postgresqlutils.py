...
SEARCH_DIRS = frozenset(["/var/run/postgresql", "/var/pgsql_socket", "/usr/local/var/postgres", "/tmp"])  # Debian default  # MacOS default  # custom compilation  # custom compilation
...
def postgres_connect(sockdir):
    ...
    try:
        return psycopg2.connect("host='%s' user='%s' password='%s' " "connect_timeout='%s' dbname=postgres" % (sockdir, user, password, CONNECT_TIMEOUT))
