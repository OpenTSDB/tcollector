...
def print_numa_stats(numafiles, ret):
    ...
    for numafilename in numafiles:
        with open(numafilename) as numafile:
            ...
            for stat, tag in (  # hit: process wanted memory from this node and got it
                              ...
                              # this one instead.
                              ("numa_miss", "miss")):
                ret.append("sys.numa.zoneallocs %d %s node=%d type=%s" % (ts, stats[stat], node_id, tag))
            ...
            ret.append("sys.numa.foreign_allocs %d %s node=%d" % (ts, stats["numa_foreign"], node_id))
            # When is memory allocated to a node that's local or remote to where
            # the process is running.
            for stat, tag in (("local_node", "local"),
                              ("other_node", "remote")):
                ret.append("sys.numa.allocation %d %s node=%d type=%s" % (ts, stats[stat], node_id, tag))
            # Pages successfully allocated with the interleave policy.
            ret.append("sys.numa.interleave %d %s node=%d type=hit" % (ts, stats["interleave_hit"], node_id))


class Procstats(CollectorBase):
    def __init__(self, config, logger):
        super(Procstats, self).__init__(config, logger)
    ...
    def __call__(self):
        ...
        for line in self.f_uptime:
            ...
            if m:
                ret_metrics.append("proc.uptime.total %d %s" % (ts, m.group(1)))
                ret_metrics.append("proc.uptime.now %d %s" % (ts, m.group(2)))
        ...
        for line in self.f_meminfo:
            ...
            if m:
                if m.group(3).lower() == 'kb':
                    ...
                    value = m.group(2)
                ret_metrics.append("proc.meminfo.%s %d %s" % (m.group(1).lower(), ts, value))
        ...
        for line in self.f_vmstat:
            ...
            if m.group(1) in ("pgpgin", "pgpgout", "pswpin",
                ...
                ret_metrics.append("proc.vmstat.%s %d %s" % (m.group(1), ts, m.group(2)))
        ...
        for line in self.f_stat:
            ...
            if m.group(1).startswith("cpu"):
                ...
                for value, field_name in zip(fields, cpu_types):
                    ret_metrics.append("proc.stat.cpu%s %d %s type=%s%s" % (metric_percpu, ts, value, field_name, tags))
            elif m.group(1) == "intr":
                ret_metrics.append("proc.stat.intr %d %s" % (ts, m.group(2).split()[0]))
            elif m.group(1) == "ctxt":
                ret_metrics.append("proc.stat.ctxt %d %s" % (ts, m.group(2)))
            elif m.group(1) == "processes":
                ret_metrics.append("proc.stat.processes %d %s" % (ts, m.group(2)))
            elif m.group(1) == "procs_blocked":
                ret_metrics.append("proc.stat.procs_blocked %d %s" % (ts, m.group(2)))
        ...
        for line in self.f_loadavg:
            ...
            if not m:
                continue
            ret_metrics.append("proc.loadavg.1min %d %s" % (ts, m.group(1)))
            ret_metrics.append("proc.loadavg.5min %d %s" % (ts, m.group(2)))
            ret_metrics.append("proc.loadavg.15min %d %s" % (ts, m.group(3)))
            ret_metrics.append("proc.loadavg.runnable %d %s" % (ts, m.group(4)))
            ret_metrics.append("proc.loadavg.total_threads %d %s" % (ts, m.group(5)))

        self.f_entropy_avail.seek(0)
        ts = int(time.time())
        for line in self.f_entropy_avail:
            ret_metrics.append("proc.kernel.entropy_avail %d %s" % (ts, line.strip()))
            ...
            if irq_type.isalnum():
                ...
                for i, val in enumerate(cols[1:]):
                    ...
                    if not val.isdigit():
                        ...
                        break
                    ret_metrics.append("proc.interrupts %s %s type=%s cpu=%s" % (ts, val, irq_type, i))
        ...
        for line in self.f_softirqs:
            ...
            for i, val in enumerate(cols[1:]):
                ...
                if not val.isdigit():
                    ...
                    break
                ret_metrics.append("proc.softirqs %s %s type=%s cpu=%s" % (ts, val, irq_type, i))

        print_numa_stats(self.numastats, ret_metrics)
        ...
        for cpu_no in self.f_scaling_min.keys():
            ...
            f.seek(0)
            for line in f:
                ret_metrics.append("proc.scaling.min %d %s cpu=%s" % (ts, line.rstrip('\n'), cpu_no))
        ts = int(time.time())
        for cpu_no in self.f_scaling_max.keys():
            f = self.f_scaling_max[cpu_no]
            f.seek(0)
            for line in f:
                ret_metrics.append("proc.scaling.max %d %s cpu=%s" % (ts, line.rstrip('\n'), cpu_no))
        ts = int(time.time())
        for cpu_no in self.f_scaling_cur.keys():
            f = self.f_scaling_cur[cpu_no]
            f.seek(0)
            for line in f:
                ret_metrics.append("proc.scaling.cur %d %s cpu=%s" % (ts, line.rstrip('\n'), cpu_no))

        return ret_metrics

if __name__ == "__main__":
    procstats = Procstats(None, None)
    procstats()
