...
# for information:
# http://haproxy.1wt.eu/download/1.4/doc/configuration.txt
METRICS_TO_REPORT = {"FRONTEND": ["scur", "stot", "rate", "req_tot", "req_rate"], "BACKEND": ["scur", "stot", "rate", "req_tot", "req_rate"], "servers": ["scur", "stot", "rate", "req_tot", "req_rate"]}
...
def print_metric(line, metric, timestamp):
    ...
    if not value:
        value = 0
    print("haproxy.%s %i %s source=%s cluster=%s" % (METRIC_NAMES[metric], timestamp, value, line["svname"], line["pxname"]))
