...
def collect(db):
  ...
  """
    ...
    try:
        ...
        cursor.execute("SELECT " "CASE WHEN pg_is_in_recovery() THEN (EXTRACT (EPOCH FROM now() - pg_last_xact_replay_timestamp()) * 1000)::BIGINT ELSE NULL END AS replication_lag_time, " "pg_xlog_location_diff(pg_last_xlog_receive_location(), pg_last_xlog_replay_location()) AS replication_lag_bytes, " "pg_is_in_recovery() AS in_recovery;")
        ts = time.time()
        stats = cursor.fetchall()

        if stats[0][0] is not None:
            print("postgresql.replication.upstream.lag.time %i %s" % (ts, stats[0][0]))

        if stats[0][1] is not None:
            print("postgresql.replication.upstream.lag.bytes %i %s" % (ts, stats[0][1]))

        print("postgresql.replication.recovering %i %i" % (ts, stats[0][2]))
        ...
        print("postgresql.replication.walreceiver.running %i %s" % (ts, wal_receiver_running))

        # WAL sender process info (could be master only or master / slave combo)
        cursor.execute("SELECT client_addr, client_port, " "pg_xlog_location_diff(sent_location, replay_location) AS lag_bytes " "FROM pg_stat_replication;")
        ts = time.time()
        stats = cursor.fetchall()

        print("postgresql.replication.downstream.count %i %i" % (ts, len(stats)))

        for stat in stats:
            print("postgresql.replication.downstream.lag.bytes %i %i client_ip=%s client_port=%s" % (ts, stat[2], stat[0], stat[1]))
