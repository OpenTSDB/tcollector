...
def convert_to_bytes(string):
    ...
    factors = {"K": 1024, "M": 1024 * 1024, "G": 1024 * 1024 * 1024, "T": 1024 * 1024 * 1024 * 1024, "P": 1024 * 1024 * 1024 * 1024 * 1024, "E": 1024 * 1024 * 1024 * 1024 * 1024 * 1024}
    if string == "-":
        return -1
...
def extract_info(line, report_disks_in_vdevs):
    (poolname, alloc, free, read_issued, write_issued, read_throughput, write_throughput) = line.split()
...
def main():
    ...
    try:
        p_zpool = subprocess.Popen(["zpool", "iostat", "-v", str(collection_interval)], stdout=subprocess.PIPE)
    except OSError as e:
    ...
    while signal_received is None:
        ...
        if start_re.match(line):
            assert ltype in (None, T_EMPTY), "expecting last state T_EMPTY or None, now got %s" % ltype
            ltype = T_START
        elif headers_re.match(line):
            assert ltype == T_START, "expecting last state T_START, now got %s" % ltype
            ltype = T_HEADERS
        elif separator_re.match(line):
            assert ltype in (T_DEVICE, T_HEADERS), "expecting last state T_DEVICE or T_HEADERS, now got %s" % ltype
            ltype = T_SEPARATOR
        elif len(line) < 2:
            assert ltype == T_SEPARATOR, "expecting last state T_SEPARATOR, now got %s" % ltype
            ltype = T_EMPTY
        elif line.startswith("  mirror"):
            assert ltype in (T_POOL, T_DEVICE), "expecting last state T_POOL or T_DEVICE, now got %s" % ltype
            ltype = T_LEG
        elif line.startswith("  "):
            assert ltype in (T_POOL, T_DEVICE, T_LEG), "expecting last state T_POOL or T_DEVICE or T_LEG, now got %s" % ltype
            ltype = T_DEVICE
        else:
            # must be a pool name
        ...
        if ltype == T_START:
            for x in (capacity_stats_pool, capacity_stats_device, io_stats_pool, io_stats_device):
                x.clear()
            timestamp = int(time.time())
            ...
            if report_capacity == report_capacity_every_x_times:
                ...
                for devicename, stats in capacity_stats_device.items():
                    ...
                    for statname, statnumber in stats.items():
                        print(fm % (statname, timestamp, statnumber, devicename, poolname))
            if firstloop:
