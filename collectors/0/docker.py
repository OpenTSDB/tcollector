...
# user 58
# system 72
proc_names = {"cpuacct.stat": ("user", "system"), "memory.stat": ("cache", "rss", "mapped_file", "pgfault", "pgmajfault", "swap", "active_anon", "inactive_anon", "active_file", "inactive_file", "unevictable", "hierarchical_memory_limit", "hierarchical_memsw_limit")}
...
# 8:1 Write 1042576
# ...
proc_names_to_agg = {"blkio.io_service_bytes": ("Read", "Write")}
...
def readdockerstats(path, containerid):
    ...
    for file_stat in os.listdir(path):
        if os.path.isfile(path + "/" + file_stat) and ((file_stat in proc_names.keys()) or (file_stat in proc_names_to_agg.keys())):
            try:
    ...
    while True:
        ...
        if os.path.isdir(CGROUP_PATH):
            for level1 in os.listdir(CGROUP_PATH):
                if (
                    os.path.isdir(CGROUP_PATH + "/" + level1 + "/docker")
                    ...
                    for slicename in ("system.slice", "machine.slice", "user.slice"):
                        if (
                            os.path.isdir(CGROUP_PATH + "/" + level1 + "/" + slicename)
                            # /cgroup/cpu and /cgroup/cpuacct are often links to /cgroup/cpu,cpuacct
