...
FIELDS_PART = ("read_issued", "read_sectors", "write_issued", "write_sectors")
...
def main():
    """iostats main loop."""
    init_stats = {"read_requests": 0, "read_merged": 0, "read_sectors": 0, "msec_read": 0, "write_requests": 0, "write_merged": 0, "write_sectors": 0, "msec_write": 0, "ios_in_progress": 0, "msec_total": 0, "msec_weighted_total": 0}
    prev_stats = dict()
    ...
    while True:
        ...
        for line in f_diskstats:
            ...
            if len(values) == 14:
                ...
                # partial stats line
                for i in range(4):
                    print("%s%s %d %s dev=%s" % (metric, FIELDS_PART[i], ts, values[i + 3], device))
            else:
