...
# We handle 4.18+, 5.5+ kernel output.
# please see definition of params here https://www.kernel.org/doc/Documentation/iostats.txt
#
...
# see definition of params here https://www.kernel.org/doc/Documentation/iostats.txt
FIELDS_DISK = (
    # kernel 2.6+
    ...
    # kernel 4.18+
    "discards_requests",  # Discards completed successfully.
    "discards_merged",  # Discards merged.
    "sectors_discarded",  # Sectors discarded successfully.
    "msecs_discard",  # Milliseconds spent discarding.

    # kernel 5.5+
    "flush_requests",  # Total number of flush requests completed successfully.
    "msecs_flushing"  # Milliseconds spent flushing
)
...
for dir_list in mapper_path_dir:
    if dir_list != 'control':
        s = os.stat(mapper_path + '/' + dir_list)
        dm2lv[(os.major(s.st_rdev), os.minor(s.st_rdev))] = dir_list
...
def main():
    ...
    while True:
        ...
        for line in f_diskstats:
            ...
            # shortcut the deduper and just skip disks that haven't done a single read.
            # This eliminates a bunch of loopback, ramdisk, and cdrom devices but still
            # lets us report on the rare case that we actually use a ramdisk.
            if values[3] == "0":
                continue
            ...
            if number_values == 18 or number_values == 20:  # see format above
                # full stats line
                # 1 major number, 2 minor number, 3 device name
                for i in range(number_values-3):
                    print("%s%s %d %s dev=%s" % (metric, FIELDS_DISK[i], ts, values[i + 3], device))
                    if FIELDS_DISK[i] == "read_sectors":
                        v = int(values[i + 3]) * get_device_sector_size(device)
                        print("%s%s %d %s dev=%s" % (metric, "read_bytes", ts, v, device))
                    if FIELDS_DISK[i] == "write_sectors":
                        v = int(values[i + 3]) * get_device_sector_size(device)
                        print("%s%s %d %s dev=%s" % (metric, "write_bytes", ts, v, device))
                ...
                if ret:
                    ...
                    util = (
                            (float(stats.get("msec_total")) - float(prev_stats[device].get("msec_total"))) *
                            float(HZ) / float(itv))
                    svctm = 0.0
                    await_ = 0.0
                    r_await = 0.0
                    ...
                    if nr_ios != prev_nr_ios:
                        await_ = (float(rd_ticks) + float(wr_ticks) - float(prev_rd_ticks) -
                                  float(prev_wr_ticks)) / float(nr_ios - prev_nr_ios)
                    print("%s%s %d %.2f dev=%s" % (metric, "svctm", ts, svctm, device))
                    print("%s%s %d %.2f dev=%s" % (metric, "r_await", ts, r_await, device))
                    print("%s%s %d %.2f dev=%s" % (metric, "w_await", ts, w_await, device))
                    print("%s%s %d %.2f dev=%s" % (metric, "await", ts, await_, device))
