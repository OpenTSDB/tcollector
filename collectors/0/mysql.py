...
DEFAULT_SOCKFILES = set(["/tmp/mysql.sock", "/var/lib/mysql/mysql.sock", "/var/run/mysqld/mysqld.sock"])  # MySQL's own default.  # RH-type / RPM systems.  # Debian-type systems.
...
class DB(object):
    ...
    def __str__(self):
        return "DB(%r, %r, version=%r)" % (self.sockfile, self.dbname, self.version)
...
def mysql_connect(sockfile):
    ...
    return MySQLdb.connect(unix_socket=sockfile, connect_timeout=CONNECT_TIMEOUT, user=user, passwd=passwd)
...
def collectInnodbStatus(db):
    ...
    m = re.search("^(\d{6}\s+\d{1,2}:\d\d:\d\d) INNODB MONITOR OUTPUT$", innodb_status, re.M)
    ...
    for line in innodb_status.split("\n"):
        ...
        if m:
            ...
            continue
        m = match("RW-shared spins (\d+), OS waits (\d+);" " RW-excl spins (\d+), OS waits (\d+)")
        if m:
...
def collect(db):
    ...
    if master_host and master_host != "None":
        ...
        printmetric("slave.thread_io_running", isyes(slave_status["slave_io_running"]))
        printmetric("slave.thread_sql_running", isyes(slave_status["slave_sql_running"]))
