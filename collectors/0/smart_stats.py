...
def alarm_handler(signum, frame):
    print("Program took too long to run, " "consider increasing its timeout.", file=sys.stderr)
    raise Alarm()
...
def is_adaptec_driver_broken():
    ...
    arcconf = subprocess.Popen("%s %s" % (ARCCONF, ARCCONF_ARGS), shell=True, stdout=subprocess.PIPE)
    ...
    for line in arcconf_output.split("\n"):
        ...
        if fields[0] == "Driver" and fields[2] in BROKEN_DRIVER_VERSIONS:
            print("arcconf indicates broken driver version %s" % fields[2], file=sys.stderr)
            return True
    return False
...
def is_3ware_driver_broken(drives):
    ...
    for i in reversed(range(len(drives))):
        drive = drives[i]
        signal.alarm(COMMAND_TIMEOUT)
        smart_ctl = subprocess.Popen(SMART_CTL + " -i /dev/" + drive, shell=True, stdout=subprocess.PIPE)
        ...
        if data_marker:
            ...
            if len(fields) > 2 and field in ATTRIBUTE_MAP:
                ...
                if is_seagate and metric in ("seek_error_rate", "raw_read_error_rate"):
                    ...
                    value = int(value)
                    print("smart.%s %d %d disk=%s" % (metric.replace("error_rate", "count"), ts, value & 0xFFFFFFFF, drive))
                    print("smart.%s %d %d disk=%s" % (metric.replace("error_rate", "errors"), ts, (value & 0xFFFF00000000) >> 32, drive))
...
def main():
    ...
    while True:
        for drive in drives:
            signal.alarm(COMMAND_TIMEOUT)
            smart_ctl = subprocess.Popen(SMART_CTL + " -i -A /dev/" + drive, shell=True, stdout=subprocess.PIPE)
            smart_output = smart_ctl.communicate()[0]
            signal.alarm(0)
