    ...
    while True:
            ...
            if irq_type.isalnum():
                ...
                for i, val in enumerate(cols[1:]):
                    ...
                    if not val.isdigit():
                        ...
                        sys.stderr.write("Unexpected interrupts value %r in" " %r: " % (val, cols))
                        break
                    print("proc.interrupts %s %s type=%s cpu=%s" % (ts, val, irq_type, i))
        ...
        for line in f_softirqs:
            ...
            for i, val in enumerate(cols[1:]):
                ...
                if not val.isdigit():
                    # something is weird, there should only be digit values
                    sys.stderr.write("Unexpected softirq value %r in" " %r: " % (val, cols))
                    break
                print("proc.softirqs %s %s type=%s cpu=%s" % (ts, val, irq_type, i))
