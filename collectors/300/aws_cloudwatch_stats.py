...
STATISTICS = frozenset(["Minimum", "Maximum", "Average", "Sum", "SampleCount"])
...
def validate_line_parses(line):
    parsed = re.match("^([-_./a-zA-Z0-9]+)\s+" "(\d+)\s+" "(\S+?)" "((?:\s+[-_./a-zA-Z0-9]+=[-_./a-zA-Z0-9]+)*)$", line)  # Metric name.  # Timestamp.  # Value (int or float).  # Tags
    if parsed is None:
