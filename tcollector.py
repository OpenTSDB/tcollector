...
if PY3:
    ...
    from http.server import HTTPServer, \
        BaseHTTPRequestHandler  # pylint: disable=maybe-no-member,no-name-in-module,import-error
    ...
    from urllib2 import Request, urlopen, HTTPError, \
        URLError  # pylint: disable=maybe-no-member,no-name-in-module,import-error
    # noinspection PyUnresolvedReferences
...
class StatusRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/" or self.path == "/status":
            ...
            LOG.debug("Handling Request for /metrics")
            result = "com_opentsdb_tcollector_reader_lines_collected{} %s" % (self.server.reader.lines_collected)
            result = "%s\ncom_opentsdb_tcollector_reader_lines_dropped{} %s" % (result, self.server.reader.lines_dropped)

            for c in self.server.collectors.values():
                result = "%s\ncom_opentsdb_tcollector_collector_lines_sent{collector=\"%s\"} %s" % (result, c.name, int(c.lines_sent))
                result = "%s\ncom_opentsdb_tcollector_collector_lines_received{collector=\"%s\"} %s" % (result, c.name, int(c.lines_received))
                result = "%s\ncom_opentsdb_tcollector_collector_lines_invalid{collector=\"%s\"} %s" % (result, c.name, int(c.lines_invalid))
                result = "%s\ncom_opentsdb_tcollector_collector_dead{collector=\"%s\"} %s" % (result, c.name, int(c.dead))
                result = "%s\ncom_opentsdb_tcollector_collector_last_datapoint{collector=\"%s\"} %s" % (result, c.name, int(c.last_datapoint * 1000))
                result = "%s\ncom_opentsdb_tcollector_collector_last_spawn{collector=\"%s\"} %s" % (result, c.name,int(c.lastspawn * 1000))

            metrics = self.server.reader.latestMetrics.getAll()
            for metricJSON in metrics:
                line = json.loads(metricJSON)
                metric = line[0].strip().replace(".", "_")
                tagString = line[3].strip()
                result_tags = ""
                if tagString != "":
                    tags = dict(item.split("=") for item in tagString.split(" "))
                    for tag in tags.keys():
                        result_tags = "%s, %s=\"%s\"" % (result_tags, tag, tags[tag])
                timestamp = line[1]
                value = line[2]
                result = "%s\n%s{%s} %s %d" % (result, metric, result_tags.lstrip(", "), value, int(timestamp * 1000))
            self.send_response(200)
...
class StatusServer(HTTPServer):
    ...
    def __init__(self, interface, port, collectors, reader):
        """
        ...
        self.reader = reader
        LOG.debug("HTTP Server listening on %s:%s" % (interface, port))
        HTTPServer.__init__(self, (interface, int(port)), StatusRequestHandler)
...
class ReaderThread(threading.Thread):
    ...
    def process_line(self, col, line):
        ...
        if not self.readerq.nput(line):
            self.lines_dropped += 1
        self.store_latest(metric, timestamp, value, tags)

    def store_latest(self, metric, timestamp, value, tags):
        # LOG.debug("adding to ItemStore: %s" % (json.dumps([metric, timestamp, value, tags])))
        self.latestMetrics.add(json.dumps([metric, timestamp, value, tags]))
...
class SenderThread(threading.Thread):
    ...
    def verify_conn(self):
        ...
        try:
            self.tsd.sendall(bytearray('version\n', 'utf-8'))
        except socket.error as msg:
            self.tsd = None
    ...
    def send_data(self):
        ...
        try:
            if self.dryrun:
                ...
                self.tsd.sendall(bytearray(out, 'utf-8'))
            self.sendq = []
        except socket.error as msg:
...
def main(argv):
    ...
    if options.monitoring_interface is not None:
        status_server = StatusServer(options.monitoring_interface, options.monitoring_port, COLLECTORS, reader)
