...
class TcollectorAgent(eossdk.AgentHandler, eossdk.SystemHandler, eossdk.TimeoutHandler):
    ...
    def _set_trace(self, level):
        level = {"debug": logging.DEBUG, "info": logging.INFO, "warn": logging.WARNING, "warning": logging.WARNING, "error": logging.ERROR}.get(level.lower())
        if not level:
            level = logging.INFO
    ...
    def _should_start(self):
        return self.module_ is not None and self.get_agent_mgr().enabled() and self._get_tsd_host()

    def _import_tcollector(self):
        if self.module_ is not None:
            return
        try:
            self.module_ = imp.load_source("tcollector", TCOLLECTOR_PATH)
    ...
    def start(self):
        ...
        args = [TCOLLECTOR_PATH, "--host", self._get_tsd_host(), "--port", str(self._get_tsd_port()), "--collector-dir=/usr/local/tcollector/collectors"]

        if self.get_agent_mgr().agent_option("dedup-interval"):
            args.append("--dedup-interval=%s" % self.get_agent_mgr().agent_option("dedup-interval"))
        ...
        reader = tcollector.ReaderThread(options.dedupinterval, options.evictinterval, options.deduponlyzero)
        ...
        sender = tcollector.SenderThread(reader, options.dryrun, hosts, not options.no_tcollector_stats, self.tags_, reconnect_interval, **kwargs)
        self.sender_thread_ = sender
        sender.start()
        debug("SenderThread startup complete")

        self.main_thread_ = threading.Thread(target=self.module_.main_loop, name="tcollector", args=(options, modules, sender, self.tags_))
