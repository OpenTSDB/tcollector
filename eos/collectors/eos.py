...
class IntfCounterCollector(object):
    ...
    def printIntfCounters(self):
        ...
        result = self.server_.runCmds(1, ["show interfaces counters", "show interfaces counters errors", "show interfaces counters bins"])
        ...
        for intf_id, intf_counters in counters["interfaces"].items():
            for counter_name, tags, eos_counter_name in counter_definitions:
                if eos_counter_name in intf_counters:
                    self.printIntfCounter(counter_name, ts, intf_counters[eos_counter_name], intf_id, tags)

        # Print interface error counters
        error_counter_definitions = [("errors", {"direction": "out"}, "outErrors"), ("errors", {"direction": "in"}, "inErrors"), ("frameTooShorts", {}, "frameTooShorts"), ("fcsErrors", {}, "fcsErrors"), ("alignmentErrors", {}, "alignmentErrors"), ("symbolErrors", {}, "symbolErrors")]
        for intf_id, intf_error_counters in error_counters["interfaceErrorCounters"].items():
            for counter_name, tags, eos_counter_name in error_counter_definitions:
                if eos_counter_name in intf_error_counters:
                    self.printIntfCounter(counter_name, ts, intf_error_counters[eos_counter_name], intf_id, tags)
        ...
        for intf_id, intf_bin_counters in bin_counters["interfaces"].items():
            for direction in ["in", "out"]:
                if not intf_bin_counters.get("%sBinsCounters" % direction):
                    continue
                for binSize in ["64", "65To127", "128To255", "256To511", "512To1023", "1024To1522", "1523ToMax"]:
    ...
    def printIntfCounter(self, counter, ts, value, intf_id, tags):
        tag_str = " ".join(["%s=%s" % (tag_name, tag_value) for (tag_name, tag_value) in tags.items()])
